...
export function Chart() {
  const [timeRange, setTimeRange] = useState<"3month" | "month" | "week">(
    "month"
  );
  ...
  const startOf3Months = format(subDays(currentDate, 90), "dd/MM/yyy");
  ...
  return (
    <Card>
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1 text-center sm:text-left">
          ...
          <CardDescription className="text-[12px] flex items-center gap-2">
            {timeRange === "3month" &&
              `Thông tin nhập xuất từ ${startOf3Months} - ${endDate}`}
            {timeRange === "month" &&
        ...
        <Select
          value={timeRange}
          onValueChange={(value) =>
            setTimeRange(value as "3month" | "month" | "week")
          }
        >
          <SelectTrigger
          ...
          <SelectContent className="rounded-xl">
            <SelectItem value="3month" className="rounded-lg text-[12px]">
              3 tháng trước
            </SelectItem>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <ChartContainer
          config={chartConfig}
          className="aspect-auto h-[45vh] w-full"
        >
